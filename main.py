...
def frequency(listob):
    ...
    freq = count(listob)
    # for shannon
    with open("./frequency/random_numbers_decimal_edition_Shannon_Output.csv", 'w') as shan:
        for i in freq:
    ...
    # for tsallis
    for i in range(2, 5):
        with open(f"./frequency/tsallis(random_numbers_decimal_edition) foimport csv
from math import log2
import os
from collections import Counter as count
# check if the file exist in current directory
...
def frequency(listob):
    ...
    # for tsallis
    for i in range(2, 5):
        with open(f"./frequency/tsallis(random_numbers_decimal_edition) for q = {i}.csv", 'w') as tsal:
            for j in freq:
...
with open("./frequency/random_numbers_decimal_edition.csv") as f:
...
with open("./frequency/random_numbers_decimal_edition_Shannon_Output.csv", 'a') as f:
    f.write("\n\n\n\nAverage: ")
    f.write(str(sum_shannon/len(inplist)))

for i in range(2, 5):
    with open(f"./frequency/tsallis(random_numbers_decimal_edition) for q = {i}.csv", 'a') as file:
        file.write("\n\n\n\nAverage: ")
